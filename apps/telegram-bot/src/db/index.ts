...
class DatabaseWrapper {
  ...
  private migrateStreaksTable(): void {
    try {
      ...
      const tableInfo = this.db.prepare("PRAGMA table_info(streaks)").all() as { name: string }[];
      const columns = tableInfo.map(col => col.name);

      if (!columns.includes('max_streak')) {
        this.db.exec(`ALTER TABLE streaks ADD COLUMN max_streak INTEGER DEFAULT 0`);
        console.log('✓ Migration: Added max_streak column');
      }

      if (!columns.includes('streak_freezes_used')) {
        this.db.exec(`ALTER TABLE streaks ADD COLUMN streak_freezes_used INTEGER DEFAULT 0`);
      ...
      if (!columns.includes('timezone')) {
        this.db.exec(`ALTER TABLE streaks ADD COLUMN timezone TEXT DEFAULT 'America/Sao_Paulo'`);
        console.log('✓ Migration: Added timezone column');
      }

      if (!columns.includes('updated_at')) {
        // SQLite doesn't support functions as default in ALTER TABLE
        this.db.exec(`ALTER TABLE streaks ADD COLUMN updated_at INTEGER DEFAULT 0`);
        // Update existing rows with current timestamp
        this.db.exec(`UPDATE streaks SET updated_at = CAST(strftime('%s', 'now') AS INTEGER) WHERE updated_at = 0`);
  ...
  // Enhanced Streaks
  getOrCreateStreak(userId: number): { user_id: number; current_streak: number; max_streak: number; last_daily_date: string | null; streak_freezes_used: number; freeze_reset_date: string | null; timezone: string } {
    ...
    ).get(userId) as { user_id: number; current_streak: number; max_streak: number; last_daily_date: string | null; streak_freezes_used: number; freeze_reset_date: string | null; timezone: string } | undefined;
    ...
    return { user_id: userId, current_streak: 0, max_streak: 0, last_daily_date: null, streak_freezes_used: 0, freeze_reset_date: null, timezone: 'America/Sao_Paulo' };
  }

  incrementStreak(userId: number): { current: number; max: number; isNewRecord: boolean } {
    const today = new Date().toISOString().split("T")[0];
    ...
    if (streak.last_daily_date === today) {
      ...
      if (streak.streak_freezes_used === 0 && this.canUseFreeze(streak.freeze_reset_date)) {
        newCurrent = streak.current_streak + 1;
  ...
  setTimezone(userId: number, timezone: string): void {
    this.db.prepare("UPDATE streaks SET timezone = ? WHERE user_id = ?").run(timezone, userId);
  }
  ...
  getTop10(): { user_id: number; max_streak: number; first_name: string }[] {
    const rows = this.db.prepare(`
      ...
      LIMIT 10
    `).all() as { user_id: number; max_streak: number; first_name: string | null }[];

    return rows.map((r, idx) => ({ ...r, first_name: r.first_name || `User ${idx + 1}` }));
  }

  getUserRank(userId: number): { rank: number; total: number; max_streak: number } | null {
    const totalRow = this.db.prepare("SELECT COUNT(*) as count FROM streaks WHERE max_streak > 0").get() as { count: number };
    ...
    return { rank: rankRow.rank, total: totalRow.count, max_streak: rankRow.max_streak };
  }

  getNearbyRanks(userId: number, range: number = 2): { rank: number; user_id: number; max_streak: number; isCurrentUser: boolean }[] {
    const userRankRow = this.db.prepare(`
    ...
    return rows.slice(start, end).map(r => ({ ...r, isCurrentUser: r.user_id === userId }));
  }
  ...
  getPendingNotifications(beforeTime: Date): { id: number; user_id: number; type: string }[] {
    return this.db.prepare(
      "SELECT id, user_id, type FROM scheduled_notifications WHERE scheduled_at <= ? AND sent_at IS NULL"
    ).all(beforeTime.toISOString()) as { id: number; user_id: number; type: string }[];
  }

  markNotificationSent(id: number, sentAt: Date): void {
    this.db.prepare("UPDATE scheduled_notifications SET sent_at = ? WHERE id = ?").run(sentAt.toISOString(), id);
  }

  clearOldNotifications(days: number = 7): void {
    const cutoff = new Date();
    cutoff.setDate(cutoff.getDate() - days);
    this.db.prepare("DELETE FROM scheduled_notifications WHERE sent_at < ?").run(cutoff.toISOString());
  }
