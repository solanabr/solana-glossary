...
import { allTerms, GlossaryTerm } from "@stbr/solana-glossary";
...
export function detectTermsInText(
  ...
  const used = new Set<number>(); // track occupied character positions

  for (const entry of termLookup) {
    if (entry.lower.length < 3) continue; // skip very short terms
    let idx = 0;
