...
export function TermPageModal({
  ...
  const handleExplainWithAI = useCallback(() => {
    sessionStorage.setItem(
      "copilot-prefill",
      `Explain the concept "${term.term}" in detail with examples`,
    );
    navigate("/copilot");
  }, [term, navigate]);
  ...
  const handleSimplify = useCallback(() => {
    sessionStorage.setItem(
      "copilot-prefill",
      `Explain "${term.term}" like I'm 5 years old (ELI5)`,
    );
    navigate("/copilot");
  }, [term, navigate]);
  ...
  const handleUseInCode = useCallback(() => {
    sessionStorage.setItem(
      "copilot-prefill",
      `Show me a real code example using "${term.term}" in Solana/Anchor`,
    );
  ...
  const handleCompare = useCallback(() => {
    ...
    sessionStorage.setItem(
      "copilot-prefill",
      `Compare "${term.term}" with "${relatedName}" in Solana`,
    );
    navigate("/copilot");
