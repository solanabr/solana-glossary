...
export function ChatUI({ onTermClick, mode = "chat" }: ChatUIProps) {
  ...
  useEffect(() => {
    if (prefillHandled.current || !aiEnabled) return;
    // Honor both the generic copilot prefill and the hero "explain this code"
    // hand-off (SmartHeroInput writes "explain-code-input").
    const prefill =
      sessionStorage.getItem("copilot-prefill") ??
      (mode === "explain-code"
        ? sessionStorage.getItem("explain-code-input")
        : null);
    if (prefill) {
      prefillHandled.current = true;
      sessionStorage.removeItem("copilot-prefill");
      sessionStorage.removeItem("explain-code-input");
